defmodule Extract.BasicTypes do
  ...
  @generate_extract_list :extracts
  ...
  # extract_statement :undefined, "undefined", [:optional] do
  ...
  #   true -> true
  #   false -> false
  #   value -> raise Error.distillation_error(value)
  # end
  ...
  # receipt_statment :number, :integer, "number to integer" do
  ...
  # receipt_statment :number, :float, "number to float" do
  #   value when is_integer(value) -> value / 1
  ...
  defmodule Meta do
    ...
    def validate_undefined(ast, ctx, :undefined, opts, body) do
      pipeline ast, ctx do
        condition Meta.is_current_format? :undefined do
          Meta.allowed_options opts, [:optional]
    ...
    defp distill_integer_to_float_comptime(ast, ctx) do
      new_ast = case ast do
        value when is_integer(value) -> value / 1
        other -> Error.comptime(ctx, distillation_error(other))
      end
      {new_ast, ctx}
    end
